...
- `PROC`内では匿名シンボル（ラベル、疑似命令の名前）として`@@, @0-@9`が使用可能です。
- `@@, @0-@9`は定義専用で`PROC`内で複数定義可能です。
- `@F, @0F-@9F`は参照専用で、記述位置より前方（下行方向）にある直近の`@@, @0-@9`を参照します。
- `@B, @0B-@9B`も参照専用で、記述位置より前方（上行方向）にある直近の`@@, @0-@9`を参照します。
- 参照位置は物理行で判定します。参照している行と同一物理行で定義されている匿名シンボルは参照できません。
...
- `"@"`で始まるマクロローカルシンボルは展開時に次のようにマングリング（名前を変更）し、グローバルシンボルとして定義します。
- `<seq>`はマクロ展開毎に振られる番です。
- `<macro_name>`は定義したマクロの名前です。
- `<name>`は定義したマクロローカルシンボルの名前です。
