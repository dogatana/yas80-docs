...
- `PROC`内では匿名シンボルとして`@@`が使用可能です。
- `@@`は定義専用のシンボルで`PROC`内で複数定義可能です。
- `@F, @B`は参照専用のシンボルで、記述位置に従って参照先の`@@`が変わります。
- `@F`は参照箇所より後に定義された直近の`@@`を参照します。
- `@B`は参照箇所より前に定義された直近の`@@`を参照します。
- 位置は物理行で判定します。参照している行と同一物理行で定義されている`@@`は参照対象外となります。
...
```
    1                                           sample proc
    2  0000 c3 03 00                 [10]           jp @f
    3  0003 3e 01                    [ 7]       @@: ld a, 1
    4  0005 c3 0b 00                 [10]           jp @f
    5  0008 c3 03 00                 [10]           jp @b
    6  000b 3e 02                    [ 7]       @@: ld a, 2
    7  000d 18 fc                    [12]           jr @b
    8                                           endp
```
...
```
if 0                     ; 0 なので次の include の（展開）内容は評価対象外
